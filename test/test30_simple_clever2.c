...
int moo(char x, int op1, int op2) {
   ...
   struct wfsptr t1_fptr;
	 t1_fptr.wfptr=&m_fptr;// 1) plus
  //  make_simple_alias(&t1_fptr,a_fptr);
   struct wfsptr t2_fptr;
	 t2_fptr.wfptr=y_fptr.wfptr;// 1) minus
  //  make_simple_alias(&t2_fptr,b_fptr);


  //  foo(0,1,&t1_fptr,&t2_fptr);
   if(op1)
   {
         struct wfsptr wftemp=t1_fptr; // 1) plus
     t1_fptr=t2_fptr;
     t2_fptr=wftemp;
    //b_fptr->wfptr->sptr->p_fptr(a,b);
   } else {
   t1_fptr.wfptr->sptr->p_fptr=minus;
	 }



		m_fptr.sptr->p_fptr(1, 1);
		y_fptr.wfptr->sptr->p_fptr(2, 2);
